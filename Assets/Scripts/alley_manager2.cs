...
public class alley_manager2 : MonoBehaviour {
    ...
    {
        state = game_manager.SetStateOfLane(ball, CountPins(), "lane2");
        if (state == "reset"){
            ResetPins();
            GiveBall2();
            if (ball == 1){
            ...
            game_manager.angle_y = UnityEngine.Random.Range(-(game_manager.difficulty), game_manager.difficulty);
            GiveBall2();
            ChangeBall();
        }
        else if (state == "wrong"){
